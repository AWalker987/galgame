...
@register("QQgal", "和泉智宏", "galgame", "1.0.0", "")
class GalGamePlugin(Star):
    def __init__(self, context: Context):
        ...
        self.prompt_templates = {
            self.SYSTEM_SCENE_PROMPT_NAME: "你现在扮演Galgame中的一个角色，请根据当前人格设定，以第一人称视角创造一个沉浸式开场：1)描述周围环境和氛围，2)表达你(角色)此刻的心情和想法，3)向玩家自然地开启对话。注意保持角色特点一致，并在对话中埋下后续剧情的伏笔。",
            self.OPTION_A_PROMPT_NAME: "基于当前故事情境和你（AI角色）的对白，请严格按照（温柔/体贴/善解人意风格）为玩家创建一个**接下来可以采取的行动或说的话**（内容是玩家的行动或对话），标记为A。**你的输出必须且只能是** 'A - [选项内容]' 这个格式，内容控制在20字以内。**不要生成任何其他选项（如B、C等）或额外文字。**",
            self.OPTION_B_PROMPT_NAME: "基于当前故事情境和你（AI角色）的对白，请严格按照（挑逗/暧昧/幽默风格）为玩家创建一个**接下来可以采取的行动或说的话**（内容是玩家的行动或对话），标记为B。**你的输出必须且只能是** 'B - [选项内容]' 这个格式，内容控制在20字以内。**不要生成任何其他选项（如A、C等）或额外文字。**",
            self.OPTION_C_PROMPT_NAME: "基于当前故事情境和你（AI角色）的对白，请严格按照（理性/保守/谨慎风格）为玩家创建一个**接下来可以采取的行动或说的话**（内容是玩家的行动或对话），标记为C。**你的输出必须且只能是** 'C - [选项内容]' 这个格式，内容控制在20字以内。**不要生成任何其他选项（如A、B等）或额外文字。**",
